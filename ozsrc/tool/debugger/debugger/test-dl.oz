...
//
class	TestDebuggerLauncher : Launchable
{
constructor:
	New
;
...
void
New( String name, String cid )
{
	Initialize() ;
	Name = (name ? name->Content() : 0) ;
	Public = (cid ? cid->Content() : 0) ;
}
...
Command( long cid, int cmd, int mode )
{
	...
	case	1:
		inline "C" {
			...
			OZ_InstanceVariable_TestDebuggerLauncher( Obj ) =
				_obj - ( _obj -> head.e -1 ) ;
...
} // class TestDebuggerLauncher [test-dl.oz]
