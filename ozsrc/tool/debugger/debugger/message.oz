...
//
//	Debugger: Message capture.
//	
// CAUTION
//	This source file is written in tabstop=4,hardtabs=8.
//
class	DebuggerMessageCapture : Launchable
{
constructor:
	New
;
...
Start() : locked
{
 global	ObjectManager	om ;
	...
	inline "C" {
		...
		argv[0] = "wish" ;
		argv[1] = "-f" ;
		argv[2] = "../../../lib/gui/debugger2/message.tcl" ;
		argv[3] = NULL ;
	...
	inline "C" {
		...
		ret = OzSetCaptureMessage( wish ) ;
	}
	if ( ret < 0 ) {
		//debug( default, "%V: OzSetCaptureMessage() = %d\n", self, ret ) ;
		debug( default, "%V: OzSetCaptureMessage()", self ) ;
		debug( default, " = %d\n", ret ) ;
		inline "C" {
			OzWrite( wish, "quit\n", 5 ) ;
			OzClose( wish ) ;
...
} // class DebuggerMessageCapture [message.oz]
