...
#
#	Mutater mudule	0.129

#
#	Akihito NAKAMURA, Electrotechnical Lab.,
#

set GADIR	"$env(OZROOT)/lib/gui/frame";
#set GADIR	"$env(HOME)/oz++/frame/frame/gui/29";
...
proc Mutate::DeleteSlide {} {
	set rv [tk_dialog .slide_delete "Slide Deletion" \
		...
		switch $att {
		  ...
		  fgcolor - bgcolor - activefgcolor - activebgcolor {
			if {"$Obj($ID.$att)" != "$Obj($copyID.$att)"} {
				lappend diff "color $att \
				  {$Obj($copyID.$att.r) $Obj($copyID.$att.g) $Obj($copyID.$att.b)}";
			}
		  }
		  default {
			if {"$Obj($ID.$att)" != "$Obj($copyID.$att)"} {
