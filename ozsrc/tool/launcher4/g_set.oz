...
class Set <Assoc<String,ProjectLinkSS>> : Collection <Assoc<String,ProjectLinkSS>>
  (rename New SuperNew;
...
/* instance variables */
    ...
    Assoc<String,ProjectLinkSS> Contents [];
...
/* method implementations */
    ...
    Assoc<String,ProjectLinkSS> Add (Assoc<String,ProjectLinkSS> t) {
    ...
    Set <Assoc<String,ProjectLinkSS>> AddContentsTo (Set <Assoc<String,ProjectLinkSS>> collection){
    ...
    Set <Assoc<String,ProjectLinkSS>> Difference (Set <Assoc<String,ProjectLinkSS>> s) {
	unsigned int i;
	Set <Assoc<String,ProjectLinkSS>> diff = ShallowCopy ();
    ...
    Assoc<String,ProjectLinkSS> DoNext (Iterator <Assoc<String,ProjectLinkSS>> pos) {
    ...
    Assoc<String,ProjectLinkSS> FindObjectWithKey (Assoc<String,ProjectLinkSS> key) {
    ...
    int IsEqual (Collection <Assoc<String,ProjectLinkSS>> s) {
	return IsSubSetOf (s) && IsSuperSetOf (s);
    }

    int IsSubSetOf (Collection <Assoc<String,ProjectLinkSS>> collection) {
    ...
    int IsSuperSetOf (Collection <Assoc<String,ProjectLinkSS>> collection) {
	if (Count < collection->Size ()) {
	    return 0;
	} else {
	    Iterator <Assoc<String,ProjectLinkSS>> i=>New (collection);
	    Assoc<String,ProjectLinkSS> c;
     ...
     */
    unsigned int OccurrencesOf (Assoc<String,ProjectLinkSS> t) {
     ...
     */
    Assoc<String,ProjectLinkSS> Remove (Assoc<String,ProjectLinkSS> t) {
	unsigned int i = FindIndexOf (t);
	Assoc<String,ProjectLinkSS> rob = Contents [i];

	if (rob == 0) {
	    raise CollectionExceptions<Assoc<String,ProjectLinkSS>>::ElementNotFound (t);
	} else {
	    unsigned int j, r;
    ...
    Assoc<String,ProjectLinkSS> RemoveAny () {
	if (Size () == 0) {
	    raise CollectionExceptions<Assoc<String,ProjectLinkSS>>::Empty;
	} else {
	    unsigned int n = length Contents, i;
	...
	if (new_size <= Count) {
	    raise
	      CollectionExceptions<Assoc<String,ProjectLinkSS>>::InvalidIntParameter (new_size);
	}
	if (Count > 0) {
	    unsigned int n = length Contents;
	    Assoc<String,ProjectLinkSS> old_contents [] = Contents;
	    Assoc<String,ProjectLinkSS> new_contents [];

	    length new_contents = SetCapacity (new_size);
	    Contents = new_contents;
	    for (i = 0; i < n; i ++) {
		Assoc<String,ProjectLinkSS> t = old_contents [i];
		if (t != 0) {
		    Add (t);
		}
	    }
    ...
    Set <Assoc<String,ProjectLinkSS>> Intersection (Set <Assoc<String,ProjectLinkSS>> s) {
	Set <Assoc<String,ProjectLinkSS>> set_intersection = ShallowCopy ();
	unsigned int n = length Contents;
	unsigned int i;
    ...
    Set <Assoc<String,ProjectLinkSS>> ShallowCopy () {
	Iterator <Assoc<String,ProjectLinkSS>> i;
	Assoc<String,ProjectLinkSS> content;
	Set <Assoc<String,ProjectLinkSS>> new=>NewWithSize (Size ());
    ...
    Set <Assoc<String,ProjectLinkSS>> Union (Set <Assoc<String,ProjectLinkSS>> s) {
	Set <Assoc<String,ProjectLinkSS>> set_union = ShallowCopy ();
...
/* protected method implementations below */
     ...
     */
    unsigned int FindIndexOf (Assoc<String,ProjectLinkSS> t) {
	unsigned int i;
	...
	debug (0, "Set <Assoc<String,ProjectLinkSS>>::H: Aw = %u, key = %u, ", Aw, key);
