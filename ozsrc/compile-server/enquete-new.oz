 ...
 protected:
  ...
  String CreateResultHTML ()
      ...
      for (i = 0; i < no_items; i++)
	{
	  ...
	  length results = no_answers;
	  max = 0;

	  for (j = 0; j < no_answers; j++)
	    {
	      results[j] = items[i]->ResultOf (j);
	  ...
	  for (j = 0; j < no_answers; j++)
		...
		result_bar[k] = '*';
	      ...
	      if (max < 40)
		{
		  result_bar[results[j]] = 0;
		}
	      else
		{
		  if (!results[j])
		    result_bar[0] = 0;
		  else if (!(results[j] * 40 / max))
		    result_bar[1] = 0;
		  else
		    result_bar[results[j] * 40 / max] = 0;
		}
