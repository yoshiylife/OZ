...
class ConfigurationDaemon : Daemon {
  constructor: New;
...
/* method implementations */
    void DaemonProcess () {
	Object cr;
	/* this is cast from a pointer to ConfigurationRequestRec */
	global VersionID vid;
	global ConfiguredClassID ccid;

	while (1) {
	    try {
		if ((cr = anExecutor->OzConfiguration ()) != 0) {
		    inline "C" {



			vid = ((OZ_ConfigurationRequest)cr)->compiled_cid;
		    }
		    ccid = OM->GetConfiguredClassID (vid, 0);
		    anExecutor->OzConfigurationReply (ccid, cr);
		}
	    } except {
		default {
