...
set path /net/oz5/lhome/yoshi/work/gui ;
...
proc	Inspect { win oid name type obj {mode remote} } \
{
	...
	"@" {
			if { [string first $type :] == -1 } {
				Unix.Send $dm "type [string range $type 1 end]" ;
				Unix.Recv $dm data ;
				Unix.Status $dm ;				# prompt
				set name [lindex $data 0] ;
				set type [lindex $data 3] ;
				set obj 0x[string range $obj 8 15] ;
				Process.Window $win $dm $oid $name "" ;
				Inspect $win.exception $oid $name $type $obj ;
			} else {
				Process.Window $win $dm $obj $name $type ;
			}
		}
	default {puts stderr "Unknown type:$type" ; flush stderr ; }
	}
}
...
#Inspect . 0f0f000e04000033 Exception @0001000002000587:000000 ffffffffee93fe30 ;
