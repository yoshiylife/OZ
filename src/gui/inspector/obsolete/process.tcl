...
# データ表示フレーム作成
	...
	Process.Update $f $f [list $dm $count $status $last] ;
...
proc	tdump { exid tid } \
{
	set cmd "debugger -R -N tdump -T $exid $tid" ;
	debug "EXEC: $cmd" ;
...
proc	Process.update { p w sid } \
{
	...
	set pid [lindex $sid 1] ;
	set tid [lindex $sid 2] ;

	$w.header.path configure -text $pid:$tid ;
	$w.header.cid configure -text $level ;

# 変数名と値のリストボックへの設定
	set flag 0 ;
	Data.Clear $w ;
	set data [tdump $pid $tid] ;
	set i 0 ;
	set count 0 ;
	foreach l $data {
		incr i
		if { $i == 4 } {
			...
			$w.list insert end "$pid:$tid.$name O$cid $self";
			set i 0 ;
			incr count ;
		}
	}
...
# データ表示フレームの更新イベント設定
	bind $w.value.listbox <Double-1> \
		"Process.Self $p $w $pid" ;

	update ;
}

proc	Process.Self { p w pid } \
{
	foreach i [$w.value.listbox curselection] {
		...
		Inspect $p.$pid:$obj $pid $name $type $obj remote ;
	}
}
