...
global dflags ;
set w .
	wm title $w "Debugger(Launch) Version 0.00" ;
...
#	wm maxsize $w 442 [winfo screenheight $w] ;
	...
	frame .dflags.body ;
	checkbutton .dflags.body.const -text Constructor -anchor nw \
		-onvalue "1" -offvalue "0" -width 12 \
		-variable dflags(constructor) -relief raised ;
	...
	pack .dflags.body.const -side top ;
...
set p "$dflags(constructor)$dflags(public)$dflags(protected)$dflags(private)" ;
		puts stdout "$id 1$p";
		flush stdout ;
	}
	if { [string compare $now "Launch"] == 0 } {
