...
if { $argc == 0 } {
	set i [string last / $argv0] ; if { $i < 0 } { set i 0 } else { incr i }
	set j [string last . $argv0] ; if { $j < 0 } { set j end } else { set j [expr $j -1] }
	set cmdName [string range $argv0 $i $j]
	puts stderr "Usage: $cmdName 0x<Process ID>"
	flush stderr
	exit 1
}
