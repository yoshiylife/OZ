...
set LIB_DIR	$env(OZROOT)/lib/gui/frame;
...
#  選択がなかったら
#   選択の種類がディレクトリならばカレントパスを選択したのか dialog で尋ねる
#   そうでなければ dialog をだして選択処理をキャンセルするか続けるか確認する
#    選択処理をキャンセルした場合には "" を OZ++ に送る。
#
...
Select ``$JSB(notify.selected)''." \
	...
	#--- 選択されていない
	if {"$JSB(notify.selected)" == "dir"} {
	    #--- 要求がディレクトリならば、current を選択したのか確認する
	    set path $JSB(path.current);
	    set ans [tk_dialog .jsb_checkselected \
			 "Junkshop Browser: SelectPath" \
			 "You have not selected any.
Do you select the current path ``$path'' ?" \
		     warning 0 "Yes" "No (retry)" "Cancel this selection"];
	    if {$ans == 0} {
		JSB::NotifySelectedPath "$path";
	    } elseif {$ans == 1} {
		#--- do nothing
...
Retry to select ``$JSB(notify.selected)''?" \
			 warning 0 "Retry" "Cancel this selection"];
	    if {$ans == 1} {
		JSB::NotifySelectedPath "";
	    }
	}
