...
set Att(all)	{ID type name state coordx coordy geom width height size \
		 ...
		 font fontsize justify value select group terminator};
...
set Att(field)		{ID type name geom size state fgcolor bgcolor \
			 terminator};
set Att(string)		{ID type name geom justify fgcolor text terminator};
...
#-----------------------------------------------------------------------
# Event (defined in "shared FEvent")
#-----------------------------------------------------------------------
set Event(ACTION)        1;
set Event(QUIT)          2;
set Event(ATTRIBUTE)     3;
set Event(DELETE)        4;
set Event(NEW)           5;
set Event(NEW_SLIDE)     6;
set Event(DELETE_SLIDE)  7;
set Event(GOTO_SLIDE)    8;
set Event(NEW_SCREEN)    9;
set Event(NEW_ITEM)     10;
set Event(DELETE_ITEM)  11;



########################################################################
#
#	OZ++ I/F --- invoked by OZ++ Objects
#
#########################################################################
...
proc OpenSeries {name width height args} {
	...
	RefreshSeries "name $name" "size $width $height";
...
proc GetField {ID} {
    global  Obj Win Event;

    set win $Win($Obj($ID.citem).path);
    set text [$win get];
    SendOZ "{TextFieldEvent $Event(ACTION) $ID \"$text\"}";
}
...
#	Debug "$att_type = $att_val";
	...
	switch $att_type {
	    ...
	    text - width - justify {
		set Obj($ID.$att_type) [lindex $att 1];
		Debug "RefreshCItem: att_type = $att_type, val = $att_val";
...
proc ButtonBrowseEvent {ID} {
    global  Obj Group Event;

    switch $Obj($ID.type) {
	button {
	    SendOZ "{ButtonEvent $Event(ACTION) $ID}";
	}
	checkbutton {
	    if {$Obj($ID.select) == 0} {
		set Obj($ID.select) 1;
		SendOZ "{CheckButtonEvent $Event(ACTION) $ID 1}";
	    } else {
		set Obj($ID.select) 0;
		SendOZ "{CheckButtonEvent $Event(ACTION) $ID 0}";
	    }
	}
	radiobutton {
	    set Group($Obj($ID.group).selected) $ID;
	    SendOZ "{RadioButtonEvent $Event(ACTION) $ID}"
	}
    }
}
...
#-----------------------------------------------------------------------
proc Core::Start {} {
    global	Frame Series Obj Event;

    #--- window title
    wm title . "OZ++/Frame(GA:$Frame(GA_version)):";
    wm protocol . WM_DELETE_WINDOW {SendOZ "{SeriesEvent $Event(QUIT)}";};
...
proc InitSeries {} {
    ...
    bind .series <Double-ButtonPress-1>	"Tool::Select browse; focus none";
...
proc Core::InitOptions {} {
#    set font -misc-fixed-bold-r-normal--14-*-*-*-*-*-iso8859-1;
#    option add *Menubutton.Font $font;
#    option add *Menu.Font $font
    option add *foreground			LightYellow;
    ...
    option add *Listbox.background		#6C6C9F;
    option add *Listbox.selectBackground	White;
    option add *Listbox.selectForeground	Red;
    ...
    option add *Checkbutton.activeBackground	#8C8CBF;
    option add *Checkbutton.activeForeground	Pink;
}
