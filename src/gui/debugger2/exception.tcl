...
set Option(Exception-ID) true ;
set Option(Exception-Param) true ;
...
proc	Capture.Message { win type date time id pid oid data } \
{
	...
	set param [lindex $data 1] ;
	set ename [lindex $data 2] ;
	set msg [lrange $data 3 end] ;

	set dm "" ;
	set data "" ;
	set exid 0x[string range $pid 4 9] ;
	Unix.Open $pid dm "-X $exid" ;
	Unix.Status $dm ;				# prompt
	if { [lindex [split $eid :] 0] != "0000000000000000" } {
		Unix.Send $dm "type [string range $eid 1 end]" ;
		Unix.Recv $dm data ;
		Unix.Status $dm ;				# prompt
		set ename [lindex $data 0] ;
	}
	if { $Capture($type) } {
		set obj 0x[string range $pid 10 end] ;
		Process.Window .process $dm $obj Exception "" ;
		if { $data != "" } {
			set	ot [lindex $data 3] ;
			set t [string index $ot 0] ;
			if { $t == "*" || $t == "R" || $t == "O" } {
					set obj 0x[string range $param 8 15] ;
				if { $obj != "0x00000000" } {
					Inspect .process.eparam $oid $ename $ot $obj $exid ;
				}
			}
		}
	} else {
	...
	if { $Option(Exception-Param) } { append head "$param " ; }

	# メッセージ表示
	Capture.message $w "$head$ename $msg\n" ;
}
