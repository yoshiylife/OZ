...
extern  int     optind, opterr ;
	...
	while ( (ch=getopt( argc, argv, "DN:RA:C:hH:iTl:" )) != EOF ) {
		switch( ch ) {
		case 'D' :
			Mode = 1 ;
			break ;
...
int
CmdDumpThreadStack( int aPort, OZ_Thread aTarget, int aIndent )
{
	int	ret = NG ;
	struct	{
		void	*addr ;
...
static	struct	regs_in {
	...
	if ( aTarget == 0 ) {
		...
		gets( buf ) ;
		aTarget = (OZ_Thread)strtol( buf, NULL, 0 ) ;
		if ( aTarget == 0 ) {
		...
		if ( addr < 0x0fffffff || cpu.r_i0 == 0 ) {
			...
			for ( i = 0 ; i < class->number_of_parts ; i ++ ) {
				...
				if ( code.base <= pc && pc <= code.base + code.size ) {
					if ( (cp=ClassCodeSymbolSearch(cid,code.base,&addr)) != NULL ) {
						name = cp ;
						part = i ;
					}
				}
...
CmdListGlobalObject( int aPort )
{
	...
	for ( i = 0 ; i < table->count ; i ++ ) {
		...
		LinePrintf( 0, "%s\n", ObjectStatusToName(table->slot[i].status) ) ;
	}
...
main( int argc, char *argv[] )
{
	...
	if ( OzClassPath == NULL ) {
		...
		int	oid = id&0x0ffffff ;
		char	buf[BUFSIZ] ;
		OzClassPath = malloc( MAXPATHLEN ) ;
		sprintf( OzClassPath, "/%s/images/%06x/classes", OzRoot, exid );
	}
	...
	{
		OZ_Thread	t = NULL ;
		if ( argind < argc ) t = (OZ_Thread)strtol( argv[argind++], NULL, 0 ) ;
		else t = (OZ_Thread)(int)(id & 0x0ffffffu) ;
