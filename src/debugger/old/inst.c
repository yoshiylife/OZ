...
InstInfo
InstInfoLoad( OZ_ClassID aRunTimeClassID )
{
static	char	*space = " \t\n" ;
	FILE	*fp ;
	char	fname[BUFSIZE] ;
	...
	if ( (fp=fopen( fname, "r" )) == NULL ) {
		perror( "fopen" ) ;
		Errorf( "Can't open instance variable debug information file '%s' !!\n", fname ) ;
		goto error ;
	}
...
InstInfo
InstInfoGet( OZ_ClassID aRunTimeClassID )
{
	InstInfoEntry	entry ;
	...
	if ( entry == NULL ) {
		...
		entry->info = InstInfoLoad( aRunTimeClassID ) ;
		if ( entry->info == NULL ) {
			free( entry ) ;
			return( NULL ) ;
		} else {
