...
ClassCodeSymbol
ClassCodeSymbolLoad( ClassID aRunTimeClassID )
{
		int	fd = -1 ;
		int	i ;
		int	size ;
...
ClassCodeSymbol
ClassCodeSymbolGet( ClassID aRunTimeClassID )
{
	...
	if ( entry == NULL ) {
		...
		entry->symbol = ClassCodeSymbolLoad( aRunTimeClassID ) ;
...
char*
ClassCodeSymbolSearch( ClassID aRunTimeClassID, unsigned long base, unsigned long *aRelAddr )
{
	struct	nlist	*symPtr ;
	struct	nlist	*symEnd;
	ClassCodeSymbol	symbol ;
...
fprintf( stderr, "Addr = 0x%08x\n", addr ) ;
	...
	symbol = ClassCodeSymbolGet( aRunTimeClassID ) ;
