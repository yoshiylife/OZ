...
OZ_ClassInfo
ClassInfoLoad( int aClass, OZ_ClassID aConfiguredClassID )
{
	...
	if ( access( fname, R_OK ) && aClass ) {
		if ( GetClassDM( aClass, aConfiguredClassID ) ) {
			Errorf( "Can't get '%s' !!\n", fname ) ;
			goto error ;
		}
	}
...
OZ_ClassInfo
ClassInfoGet( int aClass, ClassID aConfiguredClassID )
{
	...
	if ( entry == NULL ) {
		...
		entry->info = ClassInfoLoad( aClass, aConfiguredClassID ) ;
		if ( entry->info == NULL ) {
			free( entry ) ;
			return( NULL ) ;
		} else {
