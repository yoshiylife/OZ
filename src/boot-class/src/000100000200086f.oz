...
class SimpleTable <global ClassPackageID, MirroredClassPackage> {
/* interface */
...
/* instance variables */
  protected:
    ...
    global ClassPackageID KeyTable []; // Table size must be power of 2.
    MirroredClassPackage Table [];
    unsigned int Nbits;
...
/* method implementations */
    ...
    MirroredClassPackage Add (global ClassPackageID k, MirroredClassPackage v) : locked {
	MirroredClassPackage ret;
	unsigned int index;
    ...
    MirroredClassPackage AtKey (global ClassPackageID k) : locked {
	unsigned int index = FindIndexOf (k);

	if (KeyTable [index] == 0) {
	    raise CollectionExceptions <global ClassPackageID>::UnknownKey (k);
	} else {
	    return Table [index];
	}
    }

    MirroredClassPackage At (unsigned int i) : locked {
	if (KeyTable [i] == 0) {
	    return 0;
	} else {
	    return Table [i];
	...
	global ClassPackageID tmp_key_table [] = KeyTable;
	MirroredClassPackage tmp_table [] = Table;
    ...
    unsigned int FindIndexOf (global ClassPackageID k) {
	...
	raise CollectionExceptions <global ClassPackageID>::InternalError (k);
    }

    int IncludesKey (global ClassPackageID k) : locked {
    ...
    global ClassPackageID KeyAt (unsigned int i) : locked {return KeyTable [i];}

    MirroredClassPackage RemoveKey (global ClassPackageID k) : locked {
	unsigned int index = FindIndexOf (k);
	...
	if (KeyTable [index] == 0) {
	    ...
	    MirroredClassPackage ret = Table [index];
    ...
    global ClassPackageID SetOfKeys ()[] : locked {
	global ClassPackageID set [];
	unsigned int i, p;

	length set = NumberOfElement;
	for (i = 0, p = 0; p < NumberOfElement; i ++) {
	    global ClassPackageID k = KeyTable [i];
