...
class SimpleTable <global ConfigurationID, ConfigurationTable> {
/* interface */
...
/* instance variables */
  protected:
    ...
    global ConfigurationID KeyTable []; // Table size must be power of 2.
...
/* method implementations */
    ...
    ConfigurationTable Add (global ConfigurationID k, ConfigurationTable v) : locked {
	ConfigurationTable ret;
	unsigned int index;
    ...
    ConfigurationTable AtKey (global ConfigurationID k) : locked {
	unsigned int index = FindIndexOf (k);

	if (KeyTable [index] == 0) {
	    raise CollectionExceptions <global ConfigurationID>::UnknownKey (k);
	} else {
	    return Table [index];
	}
    }

    ConfigurationTable At (unsigned int i) : locked {
	if (KeyTable [i] == 0) {
	    return 0;
	} else {
	    return Table [i];
	...
	global ConfigurationID tmp_key_table [] = KeyTable;
	ConfigurationTable tmp_table [] = Table;
    ...
    unsigned int FindIndexOf (global ConfigurationID k) {
	...
	raise CollectionExceptions <global ConfigurationID>::InternalError (k);
    }

    int IncludesKey (global ConfigurationID k) : locked {
    ...
    global ConfigurationID KeyAt (unsigned int i) : locked {return KeyTable [i];}

    ConfigurationTable RemoveKey (global ConfigurationID k) : locked {
	unsigned int index = FindIndexOf (k);
	...
	if (KeyTable [index] == 0) {
	    ...
	    ConfigurationTable ret = Table [index];
    ...
    global ConfigurationID SetOfKeys ()[] : locked {
	global ConfigurationID set [];
	unsigned int i, p;

	length set = NumberOfElement;
	for (i = 0, p = 0; p < NumberOfElement; i ++) {
	    global ConfigurationID k = KeyTable [i];
