...
class Set <OIDAsKey<global DirectoryServer<T>>> : Collection <OIDAsKey<global DirectoryServer<T>>>
  (rename New SuperNew;
...
/* instance variables */
    ...
    OIDAsKey<global DirectoryServer<T>> Contents [];
...
/* method implementations */
    ...
    OIDAsKey<global DirectoryServer<T>> Add (OIDAsKey<global DirectoryServer<T>> t) {
    ...
    Set <OIDAsKey<global DirectoryServer<T>>> AddContentsTo (Set <OIDAsKey<global DirectoryServer<T>>> collection){
    ...
    Set <OIDAsKey<global DirectoryServer<T>>> Difference (Set <OIDAsKey<global DirectoryServer<T>>> s) {
	unsigned int i;
	Set <OIDAsKey<global DirectoryServer<T>>> diff = ShallowCopy ();
    ...
    OIDAsKey<global DirectoryServer<T>> DoNext (Iterator <OIDAsKey<global DirectoryServer<T>>> pos) {
    ...
    OIDAsKey<global DirectoryServer<T>> FindObjectWithKey (OIDAsKey<global DirectoryServer<T>> key) {
	return Contents [FindIndexOf (key)];
    }

    unsigned int Hash () {
	unsigned int h = 0;
	Iterator <OIDAsKey<global DirectoryServer<T>>> i;
	OIDAsKey<global DirectoryServer<T>> o;
    ...
    int IsEqual (Collection <OIDAsKey<global DirectoryServer<T>>> s) {
	return IsSubSetOf (s) && IsSuperSetOf (s);
    }

    int IsSubSetOf (Collection <OIDAsKey<global DirectoryServer<T>>> collection) {
    ...
    int IsSuperSetOf (Collection <OIDAsKey<global DirectoryServer<T>>> collection) {
	if (Count < collection->Size ()) {
	    return 0;
	} else {
	    Iterator <OIDAsKey<global DirectoryServer<T>>> i=>New (collection);
	    OIDAsKey<global DirectoryServer<T>> c;
     ...
     */
    unsigned int OccurrencesOf (OIDAsKey<global DirectoryServer<T>> t) {
     ...
     */
    OIDAsKey<global DirectoryServer<T>> Remove (OIDAsKey<global DirectoryServer<T>> t) {
	unsigned int i = FindIndexOf (t);
	OIDAsKey<global DirectoryServer<T>> rob = Contents [i];

	if (rob == 0) {
	    raise CollectionExceptions<OIDAsKey<global DirectoryServer<T>>>::ElementNotFound (t);
	} else {
	    unsigned int j, r;
    ...
    OIDAsKey<global DirectoryServer<T>> RemoveAny () {
	if (Size () == 0) {
	    raise CollectionExceptions<OIDAsKey<global DirectoryServer<T>>>::Empty;
	} else {
	    unsigned int n = length Contents, i;
	...
	if (new_size <= Count) {
	    raise
	      CollectionExceptions<OIDAsKey<global DirectoryServer<T>>>::InvalidIntParameter (new_size);
	}
	if (Count > 0) {
	    unsigned int n = length Contents;
	    OIDAsKey<global DirectoryServer<T>> old_contents [] = Contents;
	    OIDAsKey<global DirectoryServer<T>> new_contents [];

	    length new_contents = SetCapacity (new_size);
	    Contents = new_contents;
	    for (i = 0; i < n; i ++) {
		OIDAsKey<global DirectoryServer<T>> t = old_contents [i];
		if (t != 0) {
		    Add (t);
		}
	    }
    ...
    Set <OIDAsKey<global DirectoryServer<T>>> Intersection (Set <OIDAsKey<global DirectoryServer<T>>> s) {
	Set <OIDAsKey<global DirectoryServer<T>>> set_intersection = ShallowCopy ();
	unsigned int n = length Contents;
	unsigned int i;
    ...
    Set <OIDAsKey<global DirectoryServer<T>>> ShallowCopy () {
	Iterator <OIDAsKey<global DirectoryServer<T>>> i;
	OIDAsKey<global DirectoryServer<T>> content;
	Set <OIDAsKey<global DirectoryServer<T>>> new=>NewWithSize (Size ());
    ...
    Set <OIDAsKey<global DirectoryServer<T>>> Union (Set <OIDAsKey<global DirectoryServer<T>>> s) {
	Set <OIDAsKey<global DirectoryServer<T>>> set_union = ShallowCopy ();
...
/* protected method implementations below */
     ...
     */
    unsigned int FindIndexOf (OIDAsKey<global DirectoryServer<T>> t) {
	unsigned int i;
	...
	debug (0, "Set <OIDAsKey<global DirectoryServer<T>>>::H: Aw = %u, key = %u, ", Aw, key);
