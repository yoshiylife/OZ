...
class SimpleTable <global Class, FIFO<MirrorOperation>> {
/* interface */
...
/* method implementations */
    ...
    FIFO<MirrorOperation> Add (global Class k, FIFO<MirrorOperation> v) : locked {
	FIFO<MirrorOperation> ret;
	unsigned int index;
    ...
    FIFO<MirrorOperation> AtKey (global Class k) : locked {
    ...
    FIFO<MirrorOperation> At (unsigned int i) : locked {
	if (KeyTable [i] == 0) {
	    return 0;
	} else {
	    return Table [i];
	...
	FIFO<MirrorOperation> tmp_table [] = Table;
    ...
    FIFO<MirrorOperation> RemoveKey (global Class k) : locked {
	unsigned int index = FindIndexOf (k);
	...
	if (KeyTable [index] == 0) {
	    ...
	    FIFO<MirrorOperation> ret = Table [index];
