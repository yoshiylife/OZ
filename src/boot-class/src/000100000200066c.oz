...
class LockSet <global DirectoryServer<T>>
  : Set <OIDAsKey <global DirectoryServer<T>>> (rename New SuperNew;
			       rename Add SuperAdd;
...
/* instance variables */
  protected: Locked, ID;
    ...
    Set <OIDAsKey <global DirectoryServer<T>>> Locked;
    global LockID ID;
...
/* method implementations */
    ...
    global DirectoryServer<T> Add (global DirectoryServer<T> t) {
	OIDAsKey <global DirectoryServer<T>> key=>New (t);
	...
	OIDAsKey <global DirectoryServer<T>> k;
    ...
    int Includes (global DirectoryServer<T> t) {
	OIDAsKey <global DirectoryServer<T>> key=>New (t);
	...
	for (i=>New (self); (k = i->PostIncrement ()) != 0;) {
    ...
    global DirectoryServer<T> Remove (global DirectoryServer<T> t) {
	OIDAsKey <global DirectoryServer<T>> key=>New (t);
