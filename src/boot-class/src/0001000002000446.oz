...
class SimpleTable <global VersionID, global ConfiguredClassID> {
/* interface */
...
/* instance variables */
  protected:
    ...
    global VersionID KeyTable []; // Table size must be power of 2.
...
/* method implementations */
    ...
    global ConfiguredClassID Add (global VersionID k, global ConfiguredClassID v) : locked {
	global ConfiguredClassID ret;
	unsigned int index;
    ...
    global ConfiguredClassID AtKey (global VersionID k) : locked {
	unsigned int index = FindIndexOf (k);

	if (KeyTable [index] == 0) {
	    raise CollectionExceptions <global VersionID>::UnknownKey (k);
	} else {
	    return Table [index];
	}
    }
	...
	global VersionID tmp_key_table [] = KeyTable;
    ...
    unsigned int FindIndexOf (global VersionID k) {
	...
	raise CollectionExceptions <global VersionID>::InternalError (k);
    }

    int IncludesKey (global VersionID k) : locked {
    ...
    global VersionID KeyAt (unsigned int i) : locked {return KeyTable [i];}

    global ConfiguredClassID RemoveKey (global VersionID k) : locked {
	unsigned int index = FindIndexOf (k);
    ...
    global VersionID SetOfKeys ()[] : locked {
	global VersionID set [];
	unsigned int i, p;

	length set = NumberOfElement;
	for (i = 0, p = 0; p < NumberOfElement; i ++) {
	    global VersionID k = KeyTable [i];
