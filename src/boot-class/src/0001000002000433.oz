...
class SimpleTable <global ClassID, ClassPart> {
/* interface */
...
/* instance variables */
  protected:
    ...
    global ClassID KeyTable []; // Table size must be power of 2.
    ClassPart Table [];
    unsigned int Nbits;
...
/* method implementations */
    ...
    ClassPart Add (global ClassID k, ClassPart v) : locked {
	ClassPart ret;
	unsigned int index;
    ...
    ClassPart AtKey (global ClassID k) : locked {
	unsigned int index = FindIndexOf (k);

	if (KeyTable [index] == 0) {
	    raise CollectionExceptions <global ClassID>::UnknownKey (k);
	} else {
	    return Table [index];
	}
    }

    ClassPart At (unsigned int i) : locked {
	if (KeyTable [i] == 0) {
	    return 0;
	} else {
	    return Table [i];
	...
	global ClassID tmp_key_table [] = KeyTable;
	ClassPart tmp_table [] = Table;
    ...
    int IncludesKey (global ClassID k) : locked {
    ...
    global ClassID KeyAt (unsigned int i) : locked {return KeyTable [i];}

    ClassPart RemoveKey (global ClassID k) : locked {
	unsigned int index = FindIndexOf (k);
    ...
    global ClassID SetOfKeys ()[] : locked {
	global ClassID set [];
	unsigned int i, p;

	length set = NumberOfElement;
	for (i = 0, p = 0; p < NumberOfElement; i ++) {
	    global ClassID k = KeyTable [i];
