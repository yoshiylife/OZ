...
class SimpleTable <char [], SchoolValue> {
/* interface */
...
/* instance variables */
  protected:
    ...
    char KeyTable [][]; // Table size must be power of 2.
    SchoolValue Table [];
    unsigned int Nbits;
...
/* method implementations */
    ...
    SchoolValue Add (char k[], SchoolValue v) : locked {
	SchoolValue ret;
	unsigned int index;
    ...
    SchoolValue AtKey (char k[]) : locked {
	unsigned int index = FindIndexOf (k);

	if (KeyTable [index] == 0) {
	    raise CollectionExceptions <char []>::UnknownKey (k);
	} else {
	    return Table [index];
	}
    }

    SchoolValue At (unsigned int i) : locked {
	if (KeyTable [i] == 0) {
	    return 0;
	} else {
	    return Table [i];
	...
	char tmp_key_table [] []= KeyTable;
	SchoolValue tmp_table [] = Table;
	...
	raise CollectionExceptions <char []>::InternalError (k);
    }

    int IncludesKey (char k[]) : locked {
    ...
    char KeyAt (unsigned int i) []: locked {return KeyTable [i];}

    SchoolValue RemoveKey (char k[]) : locked {
	unsigned int index = FindIndexOf (k);
