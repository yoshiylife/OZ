...
class SimpleTable <global Object, ObjectTableEntry> {
/* interface */
...
/* instance variables */
  protected:
    ...
    global Object KeyTable []; // Table size must be power of 2.
    ObjectTableEntry Table [];
    unsigned int Nbits;
...
/* method implementations */
    ...
    ObjectTableEntry Add (global Object k, ObjectTableEntry v) : locked {
	ObjectTableEntry ret;
	unsigned int index;
    ...
    ObjectTableEntry AtKey (global Object k) : locked {
	unsigned int index = FindIndexOf (k);

	if (KeyTable [index] == 0) {
	    raise CollectionExceptions <global Object>::UnknownKey (k);
	} else {
	    return Table [index];
	}
    }

    ObjectTableEntry At (unsigned int i) : locked {
	if (KeyTable [i] == 0) {
	    return 0;
	} else {
	    return Table [i];
	...
	global Object tmp_key_table [] = KeyTable;
	ObjectTableEntry tmp_table [] = Table;
    ...
    unsigned int FindIndexOf (global Object k) {
	...
	raise CollectionExceptions <global Object>::InternalError (k);
    }

    int IncludesKey (global Object k) : locked {
    ...
    global Object KeyAt (unsigned int i) : locked {return KeyTable [i];}

    ObjectTableEntry RemoveKey (global Object k) : locked {
	unsigned int index = FindIndexOf (k);
	...
	if (KeyTable [index] == 0) {
	    ...
	    ObjectTableEntry ret = Table [index];
