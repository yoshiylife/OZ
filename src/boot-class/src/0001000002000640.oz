...
class Assoc <String, global ResolvableObject> {
  constructor: New;
  public: Compare, Hash, IsEqual, Key, SetKey, Value, SetValue;

/* instance variables */
    String aKey;
    global ResolvableObject aValue;

/* method implementations */
    void New (String key, global ResolvableObject value) {
	aKey = key;
	aValue = value;
    }

    int Compare (Assoc <String, global ResolvableObject> assoc) {
	return aKey->Compare (assoc->Key ());
    }

    unsigned int Hash () {return aKey->Hash ();}

    int IsEqual (Assoc <String, global ResolvableObject> assoc) {
    ...
    global ResolvableObject Value () {return aValue;}

    global ResolvableObject SetValue (global ResolvableObject new_value) {
	global ResolvableObject tmp = aValue;
	aValue = new_value;
	return tmp;
    }
}
