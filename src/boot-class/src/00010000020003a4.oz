...
/* instance variables */
  protected:
    ...
    TKey KeyTable []; // Table size must be power of 2.
    TValue Table [];
    unsigned int Nbits;
...
/* method implementations */
    ...
    TValue Add (TKey k, TValue v) : locked {
	TValue ret;
	unsigned int index;
    ...
    TValue AtKey (TKey k) : locked {
	unsigned int index = FindIndexOf (k);

	if (KeyTable [index] == 0) {
	    raise CollectionExceptions <TKey>::UnknownKey (k);
	} else {
	    return Table [index];
	}
    }
    ...
    unsigned int FindIndexOf (TKey k) {
	...
	raise CollectionExceptions <TKey>::InternalError (k);
    }

    int IncludesKey (TKey k) : locked {
    ...
    TKey KeyAt (unsigned int i) : locked {return KeyTable [i];}

    TValue RemoveKey (TKey k) : locked {
	unsigned int index = FindIndexOf (k);
