 ...
 */
class DirectoryBrowser<Catalog, Package> : GUI 
	  ...
	  rename Quit SuperQuit; )
{
 ...
 protected:
  ...
  void List (String path)
     {
       String dirs[], entries[];
       global DirectoryServer <Package> ds = aDirectory;
   ...
   void Import (String target, 
		String entry_names[], Package entries[]) : global, locked
     {
  ...
  String AddSuffix (String path, String name, Package entry)
    {
      String new;
      int i = 0;
      char num[];
  ...
  Package Retrieve (String entry_names[])[]
     {
       int i, len = length entry_names;
       Package entries[];
       String buf;
  ...
  String ListNames (Package entry)[]
    {
    }
}
