...
abstract class Collection <Linkable> {
...
/* abstract methods */
    Linkable Add (Linkable content) : abstract;
    Linkable DoNext (Iterator <Linkable> i) : abstract;
    unsigned int Hash () : abstract;
    int IsEqual (Collection <Linkable> collection) : abstract;
    unsigned int OccurrencesOf (Linkable content) : abstract;
    Linkable Remove (Linkable content) : abstract;

/* method inplementation */
    Collection <Linkable> AddAll (Collection <Linkable> collection) {
	collection->AddContentsTo (self);
	return collection;
    }

    Collection <Linkable> AddContentsTo (Collection <Linkable> collection){
	Iterator <Linkable> i;
	Linkable content;
    ...
    void DoFinish (Iterator <Linkable> i) {}

    void DoReset (Iterator <Linkable> i) {}

    int Includes (Linkable content) {return OccurrencesOf (content) != 0;}
    ...
    void NewWithCollection (Collection <Linkable> collection) {
	New ();
	AddAll (collection);
    }

    Collection <Linkable> RemoveAll (Collection <Linkable> collection) {
	Iterator <Linkable> i;
	Linkable content;
