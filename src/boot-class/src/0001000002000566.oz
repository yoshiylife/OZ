...
class Set <Assoc<String,Directory<T>>> : Collection <Assoc<String,Directory<T>>>
  (rename New SuperNew;
...
/* instance variables */
    ...
    Assoc<String,Directory<T>> Contents [];
...
/* method implementations */
    ...
    Assoc<String,Directory<T>> Add (Assoc<String,Directory<T>> t) {
    ...
    Set <Assoc<String,Directory<T>>> AddContentsTo (Set <Assoc<String,Directory<T>>> collection){
    ...
    Set <Assoc<String,Directory<T>>> Difference (Set <Assoc<String,Directory<T>>> s) {
	unsigned int i;
	Set <Assoc<String,Directory<T>>> diff = ShallowCopy ();
    ...
    Assoc<String,Directory<T>> DoNext (Iterator <Assoc<String,Directory<T>>> pos) {
    ...
    Assoc<String,Directory<T>> FindObjectWithKey (Assoc<String,Directory<T>> key) {
	return Contents [FindIndexOf (key)];
    }

    unsigned int Hash () {
	unsigned int h = 0;
	Iterator <Assoc<String,Directory<T>>> i;
	Assoc<String,Directory<T>> o;
    ...
    int IsEqual (Collection <Assoc<String,Directory<T>>> s) {
	return IsSubSetOf (s) && IsSuperSetOf (s);
    }

    int IsSubSetOf (Collection <Assoc<String,Directory<T>>> collection) {
    ...
    int IsSuperSetOf (Collection <Assoc<String,Directory<T>>> collection) {
	if (Count < collection->Size ()) {
	    return 0;
	} else {
	    Iterator <Assoc<String,Directory<T>>> i=>New (collection);
	    Assoc<String,Directory<T>> c;
     ...
     */
    unsigned int OccurrencesOf (Assoc<String,Directory<T>> t) {
     ...
     */
    Assoc<String,Directory<T>> Remove (Assoc<String,Directory<T>> t) {
	unsigned int i = FindIndexOf (t);
	Assoc<String,Directory<T>> rob = Contents [i];

	if (rob == 0) {
	    raise CollectionExceptions<Assoc<String,Directory<T>>>::ElementNotFound (t);
	} else {
	    unsigned int j, r;
    ...
    Assoc<String,Directory<T>> RemoveAny () {
	if (Size () == 0) {
	    raise CollectionExceptions<Assoc<String,Directory<T>>>::Empty;
	} else {
	    unsigned int n = length Contents, i;
	...
	if (new_size <= Count) {
	    raise
	      CollectionExceptions<Assoc<String,Directory<T>>>::InvalidIntParameter (new_size);
	}
	if (Count > 0) {
	    unsigned int n = length Contents;
	    Assoc<String,Directory<T>> old_contents [] = Contents;
	    Assoc<String,Directory<T>> new_contents [];

	    length new_contents = SetCapacity (new_size);
	    Contents = new_contents;
	    for (i = 0; i < n; i ++) {
		Assoc<String,Directory<T>> t = old_contents [i];
		if (t != 0) {
		    Add (t);
		}
	    }
    ...
    Set <Assoc<String,Directory<T>>> Intersection (Set <Assoc<String,Directory<T>>> s) {
	Set <Assoc<String,Directory<T>>> set_intersection = ShallowCopy ();
	unsigned int n = length Contents;
	unsigned int i;
    ...
    Set <Assoc<String,Directory<T>>> ShallowCopy () {
	Iterator <Assoc<String,Directory<T>>> i;
	Assoc<String,Directory<T>> content;
	Set <Assoc<String,Directory<T>>> new=>NewWithSize (Size ());
    ...
    Set <Assoc<String,Directory<T>>> Union (Set <Assoc<String,Directory<T>>> s) {
	Set <Assoc<String,Directory<T>>> set_union = ShallowCopy ();
...
/* protected method implementations below */
     ...
     */
    unsigned int FindIndexOf (Assoc<String,Directory<T>> t) {
	unsigned int i;
	...
	debug (0, "Set <Assoc<String,Directory<T>>>::H: Aw = %u, key = %u, ", Aw, key);
