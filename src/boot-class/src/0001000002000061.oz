 ...
 * class-exc.oz
 *
 * Exceptions from Class objects
 */

shared ClassExceptions {
    AlreadyVisible (global VersionID);
    InternalError ();
    InvalidVersionOrder ();
    NoDefaultVersion ();
    NoLowerVersion ();
    NotAvailableArchitecture (ArchitectureID);
    NotMirrorableCopy (global ClassID);
    NotModifiableCopy (global ClassID);
    NotVisible (global ClassID);
    NoUpperVersion ();
    UnknownClass (global ClassID);
    UnknownClassObject (global Class);
    UnknownClassPackage (global ClassPackageID);
    /* this should be moved to ObjectManagerExceptions */
    UnknownConfiguredClass (global ConfiguredClassID);
    UnknownLowerPart (global VersionID);
    UnknownPackageMember (global ClassID);
    UnknownProperty (char []);
}
