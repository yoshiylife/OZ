...
class Set <Assoc<String,T>> : Collection <Assoc<String,T>>
  (rename New SuperNew;
...
/* method implementations */
    ...
    Assoc<String,T> Add (Assoc<String,T> t) {
    ...
    Set <Assoc<String,T>> AddContentsTo (Set <Assoc<String,T>> collection){
    ...
    Set <Assoc<String,T>> Difference (Set <Assoc<String,T>> s) {
	unsigned int i;
	Set <Assoc<String,T>> diff = ShallowCopy ();
    ...
    Assoc<String,T> DoNext (Iterator <Assoc<String,T>> pos) {
    ...
    Assoc<String,T> FindObjectWithKey (Assoc<String,T> key) {
	return Contents [FindIndexOf (key)];
    }

    unsigned int Hash () {
	unsigned int h = 0;
	Iterator <Assoc<String,T>> i;
	Assoc<String,T> o;
    ...
    int IsEqual (Collection <Assoc<String,T>> s) {
	return IsSubSetOf (s) && IsSuperSetOf (s);
    }

    int IsSubSetOf (Collection <Assoc<String,T>> collection) {
    ...
    int IsSuperSetOf (Collection <Assoc<String,T>> collection) {
	if (Count < collection->Size ()) {
	    return 0;
	} else {
	    Iterator <Assoc<String,T>> i=>New (collection);
	    Assoc<String,T> c;
     ...
     */
    unsigned int OccurrencesOf (Assoc<String,T> t) {
     ...
     */
    Assoc<String,T> Remove (Assoc<String,T> t) {
	unsigned int i = FindIndexOf (t);
	Assoc<String,T> rob = Contents [i];

	if (rob == 0) {
	    raise CollectionExceptions<Assoc<String,T>>::ElementNotFound (t);
	} else {
	    unsigned int j, r;
    ...
    Assoc<String,T> RemoveAny () {
	if (Size () == 0) {
	    raise CollectionExceptions<Assoc<String,T>>::Empty;
	} else {
	    unsigned int n = length Contents, i;
	...
	if (new_size <= Count) {
	    raise
	      CollectionExceptions<Assoc<String,T>>::InvalidIntParameter (new_size);
	}
	if (Count > 0) {
	    unsigned int n = length Contents;
	    Assoc<String,T> old_contents [] = Contents;
	    Assoc<String,T> new_contents [];

	    length new_contents = SetCapacity (new_size);
	    Contents = new_contents;
	    for (i = 0; i < n; i ++) {
		Assoc<String,T> t = old_contents [i];
		if (t != 0) {
		    Add (t);
		}
	    }
    ...
    Set <Assoc<String,T>> Intersection (Set <Assoc<String,T>> s) {
	Set <Assoc<String,T>> set_intersection = ShallowCopy ();
	unsigned int n = length Contents;
	unsigned int i;
    ...
    Set <Assoc<String,T>> ShallowCopy () {
	Iterator <Assoc<String,T>> i;
	Assoc<String,T> content;
	Set <Assoc<String,T>> new=>NewWithSize (Size ());
    ...
    Set <Assoc<String,T>> Union (Set <Assoc<String,T>> s) {
	Set <Assoc<String,T>> set_union = ShallowCopy ();
...
/* protected method implementations below */
     ...
     */
    unsigned int FindIndexOf (Assoc<String,T> t) {
	unsigned int i;
	...
	debug (0, "Set <Assoc<String,T>>::H: Aw = %u, key = %u, ", Aw, key);
