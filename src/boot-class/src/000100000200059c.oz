...
class Set <Assoc<String,global DirectoryServer<T>>> : Collection <Assoc<String,global DirectoryServer<T>>>
  (rename New SuperNew;
...
/* instance variables */
    ...
    Assoc<String,global DirectoryServer<T>> Contents [];
...
/* method implementations */
    ...
    Assoc<String,global DirectoryServer<T>> Add (Assoc<String,global DirectoryServer<T>> t) {
    ...
    Set <Assoc<String,global DirectoryServer<T>>> AddContentsTo (Set <Assoc<String,global DirectoryServer<T>>> collection){
    ...
    Set <Assoc<String,global DirectoryServer<T>>> Difference (Set <Assoc<String,global DirectoryServer<T>>> s) {
	unsigned int i;
	Set <Assoc<String,global DirectoryServer<T>>> diff = ShallowCopy ();
    ...
    Assoc<String,global DirectoryServer<T>> DoNext (Iterator <Assoc<String,global DirectoryServer<T>>> pos) {
    ...
    Assoc<String,global DirectoryServer<T>> FindObjectWithKey (Assoc<String,global DirectoryServer<T>> key) {
	return Contents [FindIndexOf (key)];
    }

    unsigned int Hash () {
	unsigned int h = 0;
	Iterator <Assoc<String,global DirectoryServer<T>>> i;
	Assoc<String,global DirectoryServer<T>> o;
    ...
    int IsEqual (Collection <Assoc<String,global DirectoryServer<T>>> s) {
	return IsSubSetOf (s) && IsSuperSetOf (s);
    }

    int IsSubSetOf (Collection <Assoc<String,global DirectoryServer<T>>> collection) {
    ...
    int IsSuperSetOf (Collection <Assoc<String,global DirectoryServer<T>>> collection) {
	if (Count < collection->Size ()) {
	    return 0;
	} else {
	    Iterator <Assoc<String,global DirectoryServer<T>>> i=>New (collection);
	    Assoc<String,global DirectoryServer<T>> c;
     ...
     */
    unsigned int OccurrencesOf (Assoc<String,global DirectoryServer<T>> t) {
     ...
     */
    Assoc<String,global DirectoryServer<T>> Remove (Assoc<String,global DirectoryServer<T>> t) {
	unsigned int i = FindIndexOf (t);
	Assoc<String,global DirectoryServer<T>> rob = Contents [i];

	if (rob == 0) {
	    raise CollectionExceptions<Assoc<String,global DirectoryServer<T>>>::ElementNotFound (t);
	} else {
	    unsigned int j, r;
    ...
    Assoc<String,global DirectoryServer<T>> RemoveAny () {
	if (Size () == 0) {
	    raise CollectionExceptions<Assoc<String,global DirectoryServer<T>>>::Empty;
	} else {
	    unsigned int n = length Contents, i;
	...
	if (new_size <= Count) {
	    raise
	      CollectionExceptions<Assoc<String,global DirectoryServer<T>>>::InvalidIntParameter (new_size);
	}
	if (Count > 0) {
	    unsigned int n = length Contents;
	    Assoc<String,global DirectoryServer<T>> old_contents [] = Contents;
	    Assoc<String,global DirectoryServer<T>> new_contents [];

	    length new_contents = SetCapacity (new_size);
	    Contents = new_contents;
	    for (i = 0; i < n; i ++) {
		Assoc<String,global DirectoryServer<T>> t = old_contents [i];
		if (t != 0) {
		    Add (t);
		}
	    }
    ...
    Set <Assoc<String,global DirectoryServer<T>>> Intersection (Set <Assoc<String,global DirectoryServer<T>>> s) {
	Set <Assoc<String,global DirectoryServer<T>>> set_intersection = ShallowCopy ();
	unsigned int n = length Contents;
	unsigned int i;
    ...
    Set <Assoc<String,global DirectoryServer<T>>> ShallowCopy () {
	Iterator <Assoc<String,global DirectoryServer<T>>> i;
	Assoc<String,global DirectoryServer<T>> content;
	Set <Assoc<String,global DirectoryServer<T>>> new=>NewWithSize (Size ());
    ...
    Set <Assoc<String,global DirectoryServer<T>>> Union (Set <Assoc<String,global DirectoryServer<T>>> s) {
	Set <Assoc<String,global DirectoryServer<T>>> set_union = ShallowCopy ();
...
/* protected method implementations below */
     ...
     */
    unsigned int FindIndexOf (Assoc<String,global DirectoryServer<T>> t) {
	unsigned int i;
	...
	debug (0, "Set <Assoc<String,global DirectoryServer<T>>>::H: Aw = %u, key = %u, ", Aw, key);
