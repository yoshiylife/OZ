...
abstract class Collection <Assoc<OIDAsKey<T>,T>> {
...
/* abstract methods */
    Assoc<OIDAsKey<T>,T> Add (Assoc<OIDAsKey<T>,T> content) : abstract;
    Assoc<OIDAsKey<T>,T> DoNext (Iterator <Assoc<OIDAsKey<T>,T>> i) : abstract;
    unsigned int Hash () : abstract;
    int IsEqual (Collection <Assoc<OIDAsKey<T>,T>> collection) : abstract;
    unsigned int OccurrencesOf (Assoc<OIDAsKey<T>,T> content) : abstract;
    Assoc<OIDAsKey<T>,T> Remove (Assoc<OIDAsKey<T>,T> content) : abstract;

/* method inplementation */
    Collection <Assoc<OIDAsKey<T>,T>> AddAll (Collection <Assoc<OIDAsKey<T>,T>> collection) {
	collection->AddContentsTo (self);
	return collection;
    }

    Collection <Assoc<OIDAsKey<T>,T>> AddContentsTo (Collection <Assoc<OIDAsKey<T>,T>> collection){
    ...
    Assoc<OIDAsKey<T>,T> AsArray () [] {
	Assoc<OIDAsKey<T>,T> array [];
	Iterator <Assoc<OIDAsKey<T>,T>> i;
	Assoc<OIDAsKey<T>,T> t;
	unsigned int j = 0;
    ...
    void DoFinish (Iterator <Assoc<OIDAsKey<T>,T>> i) {}

    void DoReset (Iterator <Assoc<OIDAsKey<T>,T>> i) {}

    int Includes (Assoc<OIDAsKey<T>,T> content) {return OccurrencesOf (content) != 0;}
    ...
    void NewWithCollection (Collection <Assoc<OIDAsKey<T>,T>> collection) {
	New ();
	AddAll (collection);
    }

    Collection <Assoc<OIDAsKey<T>,T>> RemoveAll (Collection <Assoc<OIDAsKey<T>,T>> collection) {
	Iterator <Assoc<OIDAsKey<T>,T>> i;
	Assoc<OIDAsKey<T>,T> content;
