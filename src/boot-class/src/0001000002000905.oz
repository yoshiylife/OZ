...
/* method implementations */
    ...
    char At (unsigned int i) : locked {
	if (i < NumberOfElement) {
	    return Table [i];
	} else {
	    raise CollectionExceptions <char>::InvalidIntParameter (i);
	}
    }
    ...
    unsigned int Lookup (char c) : locked {return LookupImpl (c);}

    unsigned int LookupImpl (char c) {
	unsigned int i;
    ...
    char RemoveAt (unsigned int index) : locked {
	if (0 <= index && index < NumberOfElement) {
	    return RemoveAtImpl (index);
	} else {
	    raise CollectionExceptions <char>::InvalidIntParameter (index);
	}
    }

    char RemoveAtImpl (unsigned int index) {
	char ret = Table [index];
	unsigned int i;
	...
	if (elements != 0) {
	    ...
	    raise CollectionExceptions <char>::InvalidParameter;
	}
    }
