...
class Set <Assoc<T,T>> : Collection <Assoc<T,T>>
  (rename New SuperNew;
...
/* method implementations */
    ...
    Set <Assoc<T,T>> AddContentsTo (Set <Assoc<T,T>> collection){
    ...
    Set <Assoc<T,T>> Difference (Set <Assoc<T,T>> s) {
	unsigned int i;
	Set <Assoc<T,T>> diff = ShallowCopy ();
    ...
    Assoc<T,T> DoNext (Iterator <Assoc<T,T>> pos) {
    ...
    Assoc<T,T> FindObjectWithKey (Assoc<T,T> key) {
    ...
    int IsEqual (Collection <Assoc<T,T>> s) {
	return IsSubSetOf (s) && IsSuperSetOf (s);
    }

    int IsSubSetOf (Collection <Assoc<T,T>> collection) {
    ...
    int IsSuperSetOf (Collection <Assoc<T,T>> collection) {
	if (Count < collection->Size ()) {
	    return 0;
	} else {
	    Iterator <Assoc<T,T>> i=>New (collection);
	    Assoc<T,T> c;
     ...
     */
    unsigned int OccurrencesOf (Assoc<T,T> t) {
     ...
     */
    Assoc<T,T> Remove (Assoc<T,T> t) {
	unsigned int i = FindIndexOf (t);
	Assoc<T,T> rob = Contents [i];

	if (rob == 0) {
	    raise CollectionExceptions<Assoc<T,T>>::ElementNotFound (t);
	} else {
	    unsigned int j, r;
    ...
    Assoc<T,T> RemoveAny () {
	if (Size () == 0) {
	    raise CollectionExceptions<Assoc<T,T>>::Empty;
	} else {
	    unsigned int n = length Contents, i;
	...
	if (new_size <= Count) {
	    raise
	      CollectionExceptions<Assoc<T,T>>::InvalidIntParameter (new_size);
	}
	if (Count > 0) {
	    unsigned int n = length Contents;
	    Assoc<T,T> old_contents [] = Contents;
	    Assoc<T,T> new_contents [];

	    length new_contents = SetCapacity (new_size);
	    Contents = new_contents;
	    for (i = 0; i < n; i ++) {
		Assoc<T,T> t = old_contents [i];
		if (t != 0) {
		    Add (t);
		}
	    }
    ...
    Set <Assoc<T,T>> Intersection (Set <Assoc<T,T>> s) {
	Set <Assoc<T,T>> set_intersection = ShallowCopy ();
	unsigned int n = length Contents;
	unsigned int i;
    ...
    Set <Assoc<T,T>> ShallowCopy () {
	Iterator <Assoc<T,T>> i;
	Assoc<T,T> content;
	Set <Assoc<T,T>> new=>NewWithSize (Size ());
    ...
    Set <Assoc<T,T>> Union (Set <Assoc<T,T>> s) {
	Set <Assoc<T,T>> set_union = ShallowCopy ();
...
/* protected method implementations below */
     ...
     */
    unsigned int FindIndexOf (Assoc<T,T> t) {
	unsigned int i;
	...
	debug (0, "Set <Assoc<T,T>>::H: Aw = %u, key = %u, ", Aw, key);
