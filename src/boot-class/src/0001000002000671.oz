...
abstract class DirectoryServer <Package> :
...
/* instance variables */
    Dictionary <String, Directory <Package>> OwnMap;
...
/* method implementations */
    void New (Dictionary <String, global DirectoryServer <Package>> system_map,
	      Set <OIDAsKey <global DirectoryServer <Package>>> members)
      : global {
	  SuperNew ();
	...
	Directory <Package> d=>New ();
	Dictionary <String, global DirectoryServer <Package>> system_map;
	Set <OIDAsKey <global DirectoryServer <Package>>> members;
    ...
    void NewSub (Set <OIDAsKey <global DirectoryServer <Package>>> members,
		 Dictionary <String, global DirectoryServer <Package>> systemmap){
	OIDAsKey <global DirectoryServer <Package>> key=>New (oid);
    ...
    void Flush () : global {
	LockSet <global DirectoryServer <Package>> lock_set=>New ();
    ...
    void AddToMembers (global DirectoryServer <Package> ds) : global, locked {
	OIDAsKey <global DirectoryServer <Package>> key=>New (ds);

	Members->Add (key);
    }

    void AddToOwnMap (String s, Directory <Package> d) : locked {
	OwnMap->AddAssoc (s, d);
    }
    ...
    void AddToSystemMapLocally (String path, global DirectoryServer <Package> ds)
      : global, locked {
	  SystemMap->AddAssoc (path, ds);
      }

    int AskToSystemNameOwner (global NameDirectory nd) {
	global DirectoryServer <Package> ds;
	String s;
	int res;

	try {
	    ds = narrow (DirectoryServer <Package>, nd->Resolve (SystemName));
	    if (ds != 0) {
		try {
    ...
    void CheckAndExclude (Set <OIDAsKey <global DirectoryServer <Package>>> set){
	LockSet <global DirectoryServer <Package>> lock_set=>New ();
	Set <OIDAsKey <global DirectoryServer <Package>>> to_be_excluded=>New ();
	Iterator <OIDAsKey <global DirectoryServer <Package>>> i, j;
	OIDAsKey <global DirectoryServer <Package>> k, l;
	global DirectoryServer <Package> ds, m;
	int finished = 0;
	...
	LockSet <global DirectoryServer <Package>> lock_set;
	global DirectoryServer <Package> ds;
	String h;
	int finished = 0;
	...
	while (! finished) {
		...
		if (ds != oid) {
		    ...
		    if (lock_set->Lock ()) {
			Directory <Package> d = SearchOwnMap (h);
	...
	Directory <Package> d1;
	Directory <Package> d2;
    ...
    void CopyEntryTo (String path1, String path2, Package e) : global {
	Directory <Package> d = SearchOwnMap (path2);
    ...
    void CopyTreeFromTo (String path1, String path2, Directory <Package> dir) {
	if (dir != 0) {
	    Set <String> s;
	    unsigned int size;
	    ...
	    global DirectoryServer <Package> ds = DirectoryServerOfImpl (path1);
    ...
    global DirectoryServer <Package> DirectoryServerOf (String path) : global {
	LockSet <global DirectoryServer <Package>> lock_set=>New ();
	global DirectoryServer <Package> ds;
    ...
    global DirectoryServer <Package> DirectoryServerOfImpl (String path) {
	if (SearchOwnMap (path) != 0) {
	    ...
	    do {
		global DirectoryServer <Package> ds = SearchSystemMap (s);
    ...
    Set <OIDAsKey <global DirectoryServer <Package>>> DirectoryServers ()
      : global, locked {
	  return Members;
      }
	...
	Assoc <String, Directory <Package>> a;
	int changed = 0;

	for (i=>New (OwnMap); (a = i->PostIncrement ()) != 0;) {
	    Directory <Package> d = a->Value ();
	...
	while (! finished) {
	    LockSelf (lock_set);
	    try {
		OIDAsKey <global DirectoryServer <Package>> k=>New (ds);
		...
		if (Members->Includes (k)) {
			...
			for (i=>New (Members);
			     (k = i->PostIncrement ()) != 0;) {
			    global DirectoryServer <Package> m = k->Get ();
	...
	for (i=>New (Members); (k = i->PostIncrement ()) != 0;) {
	    global DirectoryServer <Package> ds = k->Get ();
    ...
    void ExcludeLocally (global DirectoryServer <Package> ds) : global {
	Iterator <Assoc <String, global DirectoryServer <Package>>> i;
	Assoc <String, global DirectoryServer <Package>> a;
    ...
    Dictionary <String, Directory <Package>> GetOwnMap () : global, locked {
	return OwnMap;
    }

    Set <String> GetOwnTops () : global, locked {return OwnTops;}

    Dictionary <String, global DirectoryServer <Package>> GetSystemMap ()
      : global, locked {
	  return SystemMap;
      }
    ...
    int IsaMember (global DirectoryServer <Package> ds) : global, locked {
	OIDAsKey <global DirectoryServer <Package>> key=>New (ds);
    ...
    void JoinImpl (global DirectoryServer <Package> ds) {
	Set <String> own_tops;
	Iterator <String> i;
	String st;
	Set <OIDAsKey <global DirectoryServer <Package>>> members;
	Iterator <OIDAsKey <global DirectoryServer <Package>>> j;
	OIDAsKey <global DirectoryServer <Package>> k;
	global DirectoryServer <Package> d;
	...
	Directory <Package> dir = SearchOwnMap (header);
	...
	try {
		...
		global DirectoryServer <Package> ds =DirectoryServerOfImpl (path);
    ...
    void LockSelf (LockSet <global DirectoryServer <Package>> lock_set) {
	lock_set->Add (oid);
	while (! lock_set->Lock ())
	  ;
    }
    ...
    global DirectoryServer <Package>
      Migrate (String path, global DirectoryServer <Package> where) : global {
	  LockSet <global DirectoryServer <Package>> lock_set=>New ();
	  Directory <Package> d;
	  int finished = 0;
	  ...
	  while (! finished) {
		  ...
		  if (d != 0) {
			  ...
			  PrepareToBeLockedGlobal (lock_set);
			  if (lock_set->Lock ()) {
			      global DirectoryServer <Package> hd;
    ...
    void MigrateFromTo (String path, Directory <Package> dir) : global {
    ...
    void MigrateFromToRecursively (String path, Directory <Package> dir) {
	if (dir != 0) {
	    Set <String> s;
	    unsigned int size;
    ...
    void MoveTreeFromTo (String path1, String path2, Directory <Package> dir) {
	...
	String h = Header (path);
	Directory <Package> d;
	global DirectoryServer <Package> ret;
    ...
    global DirectoryServer <Package>
	  ...
	  Iterator <OIDAsKey <global DirectoryServer <Package>>> i;
	  OIDAsKey <global DirectoryServer <Package>> dskey;
	  LockSet <global DirectoryServer <Package>> lock_set;
	  global DirectoryServer <Package> new;
    ...
    int Ping (global DirectoryServer <Package> ds, Waiter w) {
	try {
	    if (ds->IsReady ()) {
		w->Done ();
	    } else {
	...
	(LockSet <global DirectoryServer <Package>> lock_set, String path) {
	    Iterator <Assoc <String, global DirectoryServer <Package>>> i;
	 ...
	 String path1, String path2) {
		...
		if (d->IncludesSubdirectory (t)) {
			...
			Iterator <Assoc <String,
			                 global DirectoryServer <Package>>> i;
			Assoc <String, global DirectoryServer <Package>> assoc;
    ...
    void
      PrepareToBeLockedGlobal (LockSet<global DirectoryServer <Package>> lock_set)
	: locked {
    ...
    void
      RemoveDirectoryImpl (LockSet <global DirectoryServer <Package>> lock_set,
			   String path) {
	  Iterator <Assoc <String, global DirectoryServer <Package>>> i;
	  Assoc <String, global DirectoryServer <Package>> assoc;
	  Dictionary <String, global DirectoryServer <Package>> dic;
	  Set <Assoc <String, global DirectoryServer <Package>>> s;
	  int len = path->Length ();
	...
	Directory <Package> dir = SearchOwnMap (path);
    ...
    void RemoveFromMembers (global DirectoryServer <Package> ds) : global, locked{
	OIDAsKey <global DirectoryServer <Package>> key=>New (ds);
	...
	try {
		...
		global DirectoryServer <Package> ds = DirectoryServerOf (path);
    ...
    Directory <Package> RetrieveDirectoryLocally (String path) : global {
	return SearchOwnMap (path);
    }

    Directory <Package> SearchOwnMap (String path) : locked {
    ...
    global DirectoryServer <Package> SearchSystemMap (String path) : locked {
	...
	(Set <OIDAsKey <global DirectoryServer <Package>>> members,
	 Dictionary <String, global DirectoryServer <Package>> systemmap): global{
	     SystemMap = systemmap;
	     Members = members;
	 }
	...
	if (IsEmpty ()) {
	    Iterator <OIDAsKey <global DirectoryServer <Package>>> i;
	    OIDAsKey <global DirectoryServer <Package>> key;
    ...
    global DirectoryServer <Package> WhichDirectoryServerImpl (String path) {
	if (SearchOwnMap (path) != 0) {
	    return oid;
	} else {
	    Directory <Package> d = SearchOwnMap (Header (path));
