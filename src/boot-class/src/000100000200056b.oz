...
class Assoc <String, Directory<T>> {
  constructor: New;
...
/* method implementations */
    void New (String key, Directory<T> value) {
	aKey = key;
	aValue = value;
    }

    int Compare (Assoc <String, Directory<T>> assoc) {
	return aKey->Compare (assoc->Key ());
    }

    unsigned int Hash () {return aKey->Hash ();}

    int IsEqual (Assoc <String, Directory<T>> assoc) {
    ...
    Directory<T> Value () {return aValue;}

    Directory<T> SetValue (Directory<T> new_value) {
	Directory<T> tmp = aValue;
	aValue = new_value;
	return tmp;
    }
}
