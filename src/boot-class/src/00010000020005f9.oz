...
/* method implementations */
    void New (String key, Command value) {
	aKey = key;
	aValue = value;
    }

    int Compare (Assoc <String, Command> assoc) {
	return aKey->Compare (assoc->Key ());
    }

    unsigned int Hash () {return aKey->Hash ();}

    int IsEqual (Assoc <String, Command> assoc) {
    ...
    Command Value () {return aValue;}

    Command SetValue (Command new_value) {
	Command tmp = aValue;
	aValue = new_value;
	return tmp;
    }
