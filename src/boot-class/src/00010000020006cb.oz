...
/* method implementations */
    void New (String key, Package value) {
	aKey = key;
	aValue = value;
    }

    int Compare (Assoc <String, Package> assoc) {
	return aKey->Compare (assoc->Key ());
    }

    unsigned int Hash () {return aKey->Hash ();}

    int IsEqual (Assoc <String, Package> assoc) {
    ...
    Package Value () {return aValue;}

    Package SetValue (Package new_value) {
	Package tmp = aValue;
	aValue = new_value;
	return tmp;
    }
