...
class SimpleArray <global Class> {
/* interface */
...
/* method implementations */
	...
	if (shrink_factor > shrink_threshold) {
	    raise
	      CollectionExceptions <global Class>
    ...
    void Add (global Class c) : locked {
	AddImpl (c);
    }

    void AddImpl (global Class c) {
    ...
    void AddArray (global Class a []) : locked {
    ...
    global Class AsArray ()[] : locked {
	global Class table [] = Table;

	length table = NumberOfElement;
	return table;
    }

    global Class At (unsigned int i) : locked {
	if (i < NumberOfElement) {
	    return Table [i];
	} else {
	    raise CollectionExceptions <global Class>::InvalidIntParameter (i);
	}
    }
    ...
    global Class Content ()[] : locked {
	global Class content [] = Table;
    ...
    int Includes (global Class c) : locked {
	unsigned int i;
    ...
    unsigned int Lookup (global Class c) : locked {return LookupImpl (c);}

    unsigned int LookupImpl (global Class c) {
	unsigned int i;
    ...
    global Class Remove (global Class c) : locked {
	unsigned int index = LookupImpl (c);

	if (index != length Table && Table [index] == c) {
	    return RemoveAtImpl (index);
	} else {
	    raise CollectionExceptions <global Class>::ElementNotFound (c);
	}
    }

    global Class RemoveAt (unsigned int index) : locked {
	if (0 <= index && index < NumberOfElement) {
	    return RemoveAtImpl (index);
	} else {
	    raise CollectionExceptions <global Class>::InvalidIntParameter (index);
	}
    }

    global Class RemoveAtImpl (unsigned int index) {
	global Class ret = Table [index];
	unsigned int i;
    ...
    void Set (global Class elements []) : locked {
	global Class tmp [] = Table;
	unsigned int i, len;
	...
	if (elements != 0) {
	    ...
	    raise CollectionExceptions <global Class>::InvalidParameter;
	}
    }

    void Shrink () {
	global Class tmp_table [];
	unsigned int i;
	unsigned int capacity = length Table;
