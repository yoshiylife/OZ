...
class Set <Assoc<String,int>> : Collection <Assoc<String,int>>
  (rename New SuperNew;
...
/* method implementations */
    ...
    Assoc<String,int> Add (Assoc<String,int> t) {
    ...
    Set <Assoc<String,int>> AddContentsTo (Set <Assoc<String,int>> collection){
    ...
    Set <Assoc<String,int>> Difference (Set <Assoc<String,int>> s) {
	unsigned int i;
	Set <Assoc<String,int>> diff = ShallowCopy ();
    ...
    Assoc<String,int> DoNext (Iterator <Assoc<String,int>> pos) {
    ...
    Assoc<String,int> FindObjectWithKey (Assoc<String,int> key) {
	return Contents [FindIndexOf (key)];
    }

    unsigned int Hash () {
	unsigned int h = 0;
	Iterator <Assoc<String,int>> i;
	Assoc<String,int> o;
    ...
    int IsEqual (Collection <Assoc<String,int>> s) {
	return IsSubSetOf (s) && IsSuperSetOf (s);
    }

    int IsSubSetOf (Collection <Assoc<String,int>> collection) {
    ...
    int IsSuperSetOf (Collection <Assoc<String,int>> collection) {
	if (Count < collection->Size ()) {
	    return 0;
	} else {
	    Iterator <Assoc<String,int>> i=>New (collection);
	    Assoc<String,int> c;
     ...
     */
    unsigned int OccurrencesOf (Assoc<String,int> t) {
     ...
     */
    Assoc<String,int> Remove (Assoc<String,int> t) {
	unsigned int i = FindIndexOf (t);
	Assoc<String,int> rob = Contents [i];

	if (rob == 0) {
	    raise CollectionExceptions<Assoc<String,int>>::ElementNotFound (t);
	} else {
	    unsigned int j, r;
    ...
    Assoc<String,int> RemoveAny () {
	if (Size () == 0) {
	    raise CollectionExceptions<Assoc<String,int>>::Empty;
	} else {
	    unsigned int n = length Contents, i;
	...
	if (new_size <= Count) {
	    raise
	      CollectionExceptions<Assoc<String,int>>::InvalidIntParameter (new_size);
	}
	if (Count > 0) {
	    unsigned int n = length Contents;
	    Assoc<String,int> old_contents [] = Contents;
	    Assoc<String,int> new_contents [];

	    length new_contents = SetCapacity (new_size);
	    Contents = new_contents;
	    for (i = 0; i < n; i ++) {
		Assoc<String,int> t = old_contents [i];
		if (t != 0) {
		    Add (t);
		}
	    }
    ...
    Set <Assoc<String,int>> Intersection (Set <Assoc<String,int>> s) {
	Set <Assoc<String,int>> set_intersection = ShallowCopy ();
	unsigned int n = length Contents;
	unsigned int i;
    ...
    Set <Assoc<String,int>> ShallowCopy () {
	Iterator <Assoc<String,int>> i;
	Assoc<String,int> content;
	Set <Assoc<String,int>> new=>NewWithSize (Size ());
    ...
    Set <Assoc<String,int>> Union (Set <Assoc<String,int>> s) {
	Set <Assoc<String,int>> set_union = ShallowCopy ();
...
/* protected method implementations below */
     ...
     */
    unsigned int FindIndexOf (Assoc<String,int> t) {
	unsigned int i;
