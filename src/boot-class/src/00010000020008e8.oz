...
class SimpleTable <unsigned int, SimpleArray<Alarmable>> {
/* interface */
...
/* instance variables */
  protected:
    ...
    unsigned int KeyTable []; // Table size must be power of 2.
    SimpleArray<Alarmable> Table [];
    unsigned int Nbits;
...
/* method implementations */
    ...
    SimpleArray<Alarmable> Add (unsigned int k, SimpleArray<Alarmable> v) : locked {
	SimpleArray<Alarmable> ret;
	unsigned int index;
    ...
    SimpleArray<Alarmable> AtKey (unsigned int k) : locked {
	unsigned int index = FindIndexOf (k);

	if (KeyTable [index] == 0) {
	    raise CollectionExceptions <unsigned int>::UnknownKey (k);
	} else {
	    return Table [index];
	}
    }

    SimpleArray<Alarmable> At (unsigned int i) : locked {
	if (KeyTable [i] == 0) {
	    return 0;
	} else {
	    return Table [i];
	...
	unsigned int tmp_key_table [] = KeyTable;
	SimpleArray<Alarmable> tmp_table [] = Table;
    ...
    unsigned int FindIndexOf (unsigned int k) {
	...
	raise CollectionExceptions <unsigned int>::InternalError (k);
    }

    int IncludesKey (unsigned int k) : locked {
    ...
    unsigned int KeyAt (unsigned int i) : locked {return KeyTable [i];}

    SimpleArray<Alarmable> RemoveKey (unsigned int k) : locked {
	unsigned int index = FindIndexOf (k);
	...
	if (KeyTable [index] == 0) {
	    ...
	    SimpleArray<Alarmable> ret = Table [index];
