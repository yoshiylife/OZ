...
class SimpleTable <global Class, Waiter> {
/* interface */
...
/* method implementations */
    ...
    Waiter Add (global Class k, Waiter v) : locked {
	Waiter ret;
	unsigned int index;
    ...
    Waiter AtKey (global Class k) : locked {
    ...
    Waiter At (unsigned int i) : locked {
	if (KeyTable [i] == 0) {
	    return 0;
	} else {
	    return Table [i];
    ...
    Waiter RemoveKey (global Class k) : locked {
	unsigned int index = FindIndexOf (k);
