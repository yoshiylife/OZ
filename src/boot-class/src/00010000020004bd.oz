...
class SimpleArray <global VersionID> {
/* interface */
...
/* method implementations */
	...
	if (shrink_factor > shrink_threshold) {
	    raise
	      CollectionExceptions <global VersionID>
    ...
    void Add (global VersionID c) : locked {
	AddImpl (c);
    }

    void AddImpl (global VersionID c) {
    ...
    void AddArray (global VersionID a []) : locked {
    ...
    global VersionID AsArray ()[] : locked {
	global VersionID table [] = Table;

	length table = NumberOfElement;
	return table;
    }

    global VersionID At (unsigned int i) : locked {
	if (i < NumberOfElement) {
	    return Table [i];
	} else {
	    raise CollectionExceptions <global VersionID>::InvalidIntParameter (i);
	}
    }
    ...
    global VersionID Content ()[] : locked {
	global VersionID content [] = Table;
	...
	global VersionID tmp_table [] = Table;
    ...
    int Includes (global VersionID c) : locked {
	unsigned int i;
    ...
    unsigned int Lookup (global VersionID c) : locked {return LookupImpl (c);}

    unsigned int LookupImpl (global VersionID c) {
	unsigned int i;
    ...
    global VersionID Remove (global VersionID c) : locked {
	unsigned int index = LookupImpl (c);

	if (index != length Table && Table [index] == c) {
	    return RemoveAtImpl (index);
	} else {
	    raise CollectionExceptions <global VersionID>::ElementNotFound (c);
	}
    }

    global VersionID RemoveAt (unsigned int index) : locked {
	if (0 <= index && index < NumberOfElement) {
	    return RemoveAtImpl (index);
	} else {
	    raise CollectionExceptions <global VersionID>::InvalidIntParameter (index);
	}
    }

    global VersionID RemoveAtImpl (unsigned int index) {
	global VersionID ret = Table [index];
	unsigned int i;
    ...
    void Set (global VersionID elements []) : locked {
	global VersionID tmp [] = Table;
	unsigned int i, len;
	...
	if (elements != 0) {
	    ...
	    raise CollectionExceptions <global VersionID>::InvalidParameter;
	}
    }

    void Shrink () {
	global VersionID tmp_table [];
	unsigned int i;
	unsigned int capacity = length Table;
