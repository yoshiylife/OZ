...
shared CollectionExceptions <OIDAsKey<global DirectoryServer<T>>> {
    ElementNotFound (OIDAsKey<global DirectoryServer<T>>  );
    Empty ();
    InternalError (OIDAsKey<global DirectoryServer<T>>  );
    InvalidIntParameter (int);
    InvalidParameter ();
    RedefinitionOfKey (OIDAsKey<global DirectoryServer<T>>  );
    UnknownKey (OIDAsKey<global DirectoryServer<T>>  );
}
