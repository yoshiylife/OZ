...
class SimpleArray <global ClassPackageID> {
/* interface */
...
/* method implementations */
	...
	if (shrink_factor > shrink_threshold) {
	    raise
	      CollectionExceptions <global ClassPackageID>
    ...
    void Add (global ClassPackageID c) : locked {
	AddImpl (c);
    }

    void AddImpl (global ClassPackageID c) {
    ...
    void AddArray (global ClassPackageID a []) : locked {
    ...
    global ClassPackageID AsArray ()[] : locked {
	global ClassPackageID table [] = Table;

	length table = NumberOfElement;
	return table;
    }

    global ClassPackageID At (unsigned int i) : locked {
	if (i < NumberOfElement) {
	    return Table [i];
	} else {
	    raise CollectionExceptions <global ClassPackageID>::InvalidIntParameter (i);
	}
    }
    ...
    global ClassPackageID Content ()[] : locked {
	global ClassPackageID content [] = Table;
	...
	global ClassPackageID tmp_table [] = Table;
    ...
    int Includes (global ClassPackageID c) : locked {
	unsigned int i;
    ...
    unsigned int Lookup (global ClassPackageID c) : locked {return LookupImpl (c);}

    unsigned int LookupImpl (global ClassPackageID c) {
	unsigned int i;
    ...
    global ClassPackageID Remove (global ClassPackageID c) : locked {
	unsigned int index = LookupImpl (c);

	if (index != length Table && Table [index] == c) {
	    return RemoveAtImpl (index);
	} else {
	    raise CollectionExceptions <global ClassPackageID>::ElementNotFound (c);
	}
    }

    global ClassPackageID RemoveAt (unsigned int index) : locked {
	if (0 <= index && index < NumberOfElement) {
	    return RemoveAtImpl (index);
	} else {
	    raise CollectionExceptions <global ClassPackageID>::InvalidIntParameter (index);
	}
    }

    global ClassPackageID RemoveAtImpl (unsigned int index) {
	global ClassPackageID ret = Table [index];
	unsigned int i;
    ...
    void Set (global ClassPackageID elements []) : locked {
	global ClassPackageID tmp [] = Table;
	unsigned int i, len;
	...
	if (elements != 0) {
	    ...
	    raise CollectionExceptions <global ClassPackageID>::InvalidParameter;
	}
    }

    void Shrink () {
	global ClassPackageID tmp_table [];
	unsigned int i;
	unsigned int capacity = length Table;
