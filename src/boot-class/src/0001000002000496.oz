...
class Set <OIDAsKey<T>> : Collection <OIDAsKey<T>>
  (rename New SuperNew;
...
/* method implementations */
    ...
    Set <OIDAsKey<T>> AddContentsTo (Set <OIDAsKey<T>> collection){
    ...
    Set <OIDAsKey<T>> Difference (Set <OIDAsKey<T>> s) {
	unsigned int i;
	Set <OIDAsKey<T>> diff = ShallowCopy ();
    ...
    OIDAsKey<T> DoNext (Iterator <OIDAsKey<T>> pos) {
    ...
    OIDAsKey<T> FindObjectWithKey (OIDAsKey<T> key) {
    ...
    int IsEqual (Collection <OIDAsKey<T>> s) {
	return IsSubSetOf (s) && IsSuperSetOf (s);
    }

    int IsSubSetOf (Collection <OIDAsKey<T>> collection) {
    ...
    int IsSuperSetOf (Collection <OIDAsKey<T>> collection) {
	if (Count < collection->Size ()) {
	    return 0;
	} else {
	    Iterator <OIDAsKey<T>> i=>New (collection);
	    OIDAsKey<T> c;
     ...
     */
    unsigned int OccurrencesOf (OIDAsKey<T> t) {
     ...
     */
    OIDAsKey<T> Remove (OIDAsKey<T> t) {
	unsigned int i = FindIndexOf (t);
	OIDAsKey<T> rob = Contents [i];

	if (rob == 0) {
	    raise CollectionExceptions<OIDAsKey<T>>::ElementNotFound (t);
	} else {
	    unsigned int j, r;
	...
	if (new_size <= Count) {
	    raise
	      CollectionExceptions<OIDAsKey<T>>::InvalidIntParameter (new_size);
	}
	if (Count > 0) {
	    unsigned int n = length Contents;
	    OIDAsKey<T> old_contents [] = Contents;
	    OIDAsKey<T> new_contents [];

	    length new_contents = SetCapacity (new_size);
	    Contents = new_contents;
	    for (i = 0; i < n; i ++) {
		OIDAsKey<T> t = old_contents [i];
		if (t != 0) {
		    Add (t);
		}
	    }
    ...
    Set <OIDAsKey<T>> Intersection (Set <OIDAsKey<T>> s) {
	Set <OIDAsKey<T>> set_intersection = ShallowCopy ();
	unsigned int n = length Contents;
	unsigned int i;
    ...
    Set <OIDAsKey<T>> ShallowCopy () {
	Iterator <OIDAsKey<T>> i;
	OIDAsKey<T> content;
	Set <OIDAsKey<T>> new=>NewWithSize (Size ());
    ...
    Set <OIDAsKey<T>> Union (Set <OIDAsKey<T>> s) {
	Set <OIDAsKey<T>> set_union = ShallowCopy ();
...
/* protected method implementations below */
     ...
     */
    unsigned int FindIndexOf (OIDAsKey<T> t) {
	unsigned int i;
	...
	debug (0, "Set <OIDAsKey<T>>::H: Aw = %u, key = %u, ", Aw, key);
