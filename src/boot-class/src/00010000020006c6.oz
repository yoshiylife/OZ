...
class Set <Assoc<String,Package>> : Collection <Assoc<String,Package>>
  (rename New SuperNew;
...
/* method implementations */
    ...
    Assoc<String,Package> Add (Assoc<String,Package> t) {
    ...
    Set <Assoc<String,Package>> AddContentsTo (Set <Assoc<String,Package>> collection){
    ...
    Set <Assoc<String,Package>> Difference (Set <Assoc<String,Package>> s) {
	unsigned int i;
	Set <Assoc<String,Package>> diff = ShallowCopy ();
    ...
    Assoc<String,Package> DoNext (Iterator <Assoc<String,Package>> pos) {
    ...
    Assoc<String,Package> FindObjectWithKey (Assoc<String,Package> key) {
	return Contents [FindIndexOf (key)];
    }

    unsigned int Hash () {
	unsigned int h = 0;
	Iterator <Assoc<String,Package>> i;
	Assoc<String,Package> o;
    ...
    int IsEqual (Collection <Assoc<String,Package>> s) {
	return IsSubSetOf (s) && IsSuperSetOf (s);
    }

    int IsSubSetOf (Collection <Assoc<String,Package>> collection) {
    ...
    int IsSuperSetOf (Collection <Assoc<String,Package>> collection) {
	if (Count < collection->Size ()) {
	    return 0;
	} else {
	    Iterator <Assoc<String,Package>> i=>New (collection);
	    Assoc<String,Package> c;
     ...
     */
    unsigned int OccurrencesOf (Assoc<String,Package> t) {
     ...
     */
    Assoc<String,Package> Remove (Assoc<String,Package> t) {
	unsigned int i = FindIndexOf (t);
	Assoc<String,Package> rob = Contents [i];

	if (rob == 0) {
	    raise CollectionExceptions<Assoc<String,Package>>::ElementNotFound (t);
	} else {
	    unsigned int j, r;
    ...
    Assoc<String,Package> RemoveAny () {
	if (Size () == 0) {
	    raise CollectionExceptions<Assoc<String,Package>>::Empty;
	} else {
	    unsigned int n = length Contents, i;
	...
	if (new_size <= Count) {
	    raise
	      CollectionExceptions<Assoc<String,Package>>::InvalidIntParameter (new_size);
	}
	if (Count > 0) {
	    unsigned int n = length Contents;
	    Assoc<String,Package> old_contents [] = Contents;
	    Assoc<String,Package> new_contents [];

	    length new_contents = SetCapacity (new_size);
	    Contents = new_contents;
	    for (i = 0; i < n; i ++) {
		Assoc<String,Package> t = old_contents [i];
		if (t != 0) {
		    Add (t);
		}
	    }
    ...
    Set <Assoc<String,Package>> Intersection (Set <Assoc<String,Package>> s) {
	Set <Assoc<String,Package>> set_intersection = ShallowCopy ();
	unsigned int n = length Contents;
	unsigned int i;
    ...
    Set <Assoc<String,Package>> ShallowCopy () {
	Iterator <Assoc<String,Package>> i;
	Assoc<String,Package> content;
	Set <Assoc<String,Package>> new=>NewWithSize (Size ());
    ...
    Set <Assoc<String,Package>> Union (Set <Assoc<String,Package>> s) {
	Set <Assoc<String,Package>> set_union = ShallowCopy ();
...
/* protected method implementations below */
     ...
     */
    unsigned int FindIndexOf (Assoc<String,Package> t) {
	unsigned int i;
