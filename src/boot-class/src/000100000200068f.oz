...
class Assoc <String, Directory<Package>> {
  constructor: New;
  public: Compare, Hash, IsEqual, Key, SetKey, Value, SetValue;

/* instance variables */
    String aKey;
    Directory<Package> aValue;

/* method implementations */
    void New (String key, Directory<Package> value) {
	aKey = key;
	aValue = value;
    }

    int Compare (Assoc <String, Directory<Package>> assoc) {
	return aKey->Compare (assoc->Key ());
    }

    unsigned int Hash () {return aKey->Hash ();}

    int IsEqual (Assoc <String, Directory<Package>> assoc) {
    ...
    Directory<Package> Value () {return aValue;}

    Directory<Package> SetValue (Directory<Package> new_value) {
	Directory<Package> tmp = aValue;
	aValue = new_value;
	return tmp;
    }
}
