...
char	*ataddr;
	...
	if((ataddr = (char *)shmat(shmid, (char*)0, 0)) == (char *)(-1)) {
		(void)fprintf(stderr, "gt_shmaddr: shmat failed\n");
		exit(1);
	}
	return(ataddr);
}
...
static int	rm_shmaddr(int shmid, char *ataddr)
{
	...
	*/
	if(shmdt(ataddr)) {
		printf(stderr, "rm_shmaddr: shmdt failed\n");
		return((int)(-1));
	}
	/*
	*  Shared memory dettach
...
void	rm_shmmem(key_t key)
{
int	shmid;

	shmid = shmget(key, 0, 0666);
	shmctl(shmid, IPC_RMID, (struct shmid_ds *)0);
	shmid = shmget(key + 1, 0, 0666);
	shmctl(shmid, IPC_RMID, (struct shmid_ds *)0);
}
...
char	*addr;
	...
	if(shm_cnt == MAX_OF_SHMTBL) {
		(void)printf("alloc_shmmem: shm table over\n");
		return((char *)(-1));
	}
	if(shm_cnt == 0) {
	...
	if(shm_table[shm_cnt].shmid == (-1)) {
		(void)printf("alloc_shmmem: get shmid failed\n");
		return((char *)(-1));
	}
