...
OzInitETHash(ETHashTable h)
{
  ...
  h->mask = ET_HASH_MASK;
  h->count = 0;
  bzero((char *)h->table, sizeof(ETHashTableItemRec)*ET_HASH_INDEX_SIZE);
  return;
}
...
int	
OzEnterETHash(ETHashTable hash, long long id,int val)
{
	int	h;
...
#ifndef _NCL_CODE_
int
  OzSearchETHash(ETHashTable hash, long long id)
{
  int	h,prev_lock;
  ETHashTableItem item;
  int val;
 ...
 search_again:
  ...
  for (h = ETHash_val(id, hash->mask),item = &(hash->table[h]);
       (item->id) != 0;
       h = ((h + INDEX_HASH_SKIP) & hash->mask),item = &(hash->table[h]))
    if (item->id == id)
      {
	extbl[item->val].lastaccess=prev_lock;
	break;
      }
  ...
  if(prev_lock == hash->lock)
    {
      if( (item->id) == id)
	{ /*OzDebugf("OzSearchETHash returns %d\n",item->val);*/
	  val=item->val;
	  return(val); }
      else
	return(-1);
...
int
OzSearchETHashNcl(ETHashTable hash, long long id)
{
  int	h;
...
void
OzRemoveETHash(ETHashTable hash, long long id)
{
  struct {
    long long id;
    int val;
